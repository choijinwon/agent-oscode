...
입력 패널은 현재 모드·모델과 전송 안내를 한 영역에 표시합니다. 긴 입력은 터미널 폭에 따라 자동으로 줄바꿈되며, 창 크기를 바꿔도 입력 내용과 편집 커서를 유지합니다. 입력 중 스크롤은 터미널의 커서 추적 동작을 사용합니다. 사용자가 과거 로그로 스크롤한 경우의 화면 복귀는 터미널 설정에 따라 다릅니다. 여러 줄 코드 붙여넣기는 `/paste` → `/send`를 사용하세요.
