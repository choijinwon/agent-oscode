...
`/design erp react`는 회계 전표 조회 테이블과 편집형 그리드를 제공합니다. 회사·통화별 합계, 엑셀 셀 붙여넣기, 차대변 검증과 초안 이벤트를 지원합니다. SAP 서버 연결·실제 전기는 앱에서 구현합니다. [ERP 가이드](docs/design-studio.md#회계erp-테이블과-그리드).
