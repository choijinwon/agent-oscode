...
`/design gallery react`로 로컬 브라우저에서 컴포넌트를 검색하고 색상·크기·스타일·상태별 예시를 살펴봅니다. 폼·탭·모달·툴팁·테이블과 모바일·관리자 디자인 등 28종을 React·Vue·Angular·Svelte 코드로 생성하며, 프로젝트 토큰과 팀 컴포넌트 공유 파일을 지원합니다.
