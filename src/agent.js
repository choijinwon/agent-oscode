...
export async function runTurn({ session, prompt, config, provider, tools, signal, emit = () => {}, save = async () => {}, executionPlan = null }) {
  ...
  try {
    ...
    const definitions = toolDefinitions.filter(t => (agent === 'frontend' || !['frontend_inspect', 'ui_check', 'ui_component', 'frontend_architecture'].includes(t.name)) && (!config.plan || planReadTools.has(t.name)));
