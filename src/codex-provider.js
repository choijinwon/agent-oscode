...
export function createCodexProvider({connect=startCodex}={}) {
 return {async complete(request,signal) {
  ...
  try{
   ...
   const started=await client.request('turn/start',{threadId,input:[{type:'text',text:JSON.stringify({conversation:request.messages,availableTools:request.tools,outputTokenTarget:request.maxOutput}),text_elements:[]}],environments:[],sandboxPolicy:{type:'readOnly',access:{type:'restricted',includePlatformDefaults:false,readableRoots:[client.cwd]}},approvalPolicy:'never',outputSchema:schema},signal);
