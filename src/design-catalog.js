...
export function designCss(tokens=defaultTheme,scope="",mobile=false,admin=false,interaction=false,erp=false){return `.oc-design${scope?`[data-design="${scope}"]`:""}{${themeCss(tokens)}}\n${designStyles}${designMotionStyles}${mobile?mobileStyles:''}${admin?adminStyles:''}${interaction?patternStyles:''}${erp?erpStyles:''}`;}
