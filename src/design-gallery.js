...
export function galleryClient(data){
 ...
 let selected=data.initial||'button',search=data.initial?.startsWith('admin-')?'관리자':'',variant='solid',size='medium',framework=data.framework||'react',viewport=data.initial==='mobile-tabs'?'390':'fit',frame=null;const tokens={...data.theme};
 $('#search').value=search;
 ...
 function makePreview(id,state,uid){
  ...
  for(const type of ['click','keydown','submit','input','change','focusin','focusout','mouseout'])host.addEventListener(type,event=>{const emit=()=>{$('#activity').textContent='미리보기 동작을 확인했습니다. 실제 저장·인증 요청은 보내지 않습니다.';};designAction(event,emit);adminAction(event,emit);});
  if(state==='disabled'||state==='loading')host.querySelectorAll('button,input,select').forEach(el=>el.disabled=true);
 ...
 function mobilePreview(item){
  ...
  frame.srcdoc=mobilePreviewHtml(data.markup[selected],'.oc-design{'+vars(tokens)+'}'+data.styles,{variant,size,mobile:item.platform==='mobile-web',admin:item.platform==='admin-web',safeArea:$('#safe-area').checked,parentOrigin:location.origin},data.nonce);
