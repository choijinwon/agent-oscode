...
export function galleryClient(data){
 ...
 let selected=data.initial||'button',search=data.initial?.startsWith('admin-')?'관리자':'',variant='solid',size='medium',motion='auto',framework=data.framework||'react',viewport=data.initial==='mobile-tabs'?'390':'fit',frame=null;const tokens={...data.theme};
 $('#search').value=search;
 ...
 function mobilePreview(item){
  ...
  frame.srcdoc=mobilePreviewHtml(data.markup[selected],'.oc-design{'+vars(tokens)+'}'+data.styles,{variant,size,motion,mobile:item.platform==='mobile-web',admin:item.platform==='admin-web',safeArea:$('#safe-area').checked,parentOrigin:location.origin},data.nonce);
