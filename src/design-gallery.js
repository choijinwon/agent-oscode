...
export function galleryClient(data){
 ...
 let selected='button',search='',variant='solid',size='medium',framework=data.framework||'react';const tokens={...data.theme};
 ...
 function renderList(){
  list.replaceChildren();for(const item of data.items.filter(x=>(x.name+' '+x.description+' '+x.group).toLowerCase().includes(search))){
   ...
   const title=document.createElement('strong'),caption=document.createElement('span');title.textContent=item.name;caption.textContent=item.description;button.append(title,caption);button.onclick=()=>{selected=item.id;render();};list.append(button);
  }
 }
 function makePreview(id,state,uid){
  const host=document.createElement('section');host.className='oc-design';host.dataset.variant=variant;host.dataset.size=size;host.setAttribute('style',vars(state==='dark'?{...data.dark,accent:tokens.accent,accentText:tokens.accentText}:tokens));
  host.innerHTML=data.markup[id].replaceAll('__id__',uid);
  for(const type of ['click','keydown','submit','input','focusout','mouseout'])host.addEventListener(type,event=>designAction(event,()=>{$('#activity').textContent='미리보기 동작을 확인했습니다. 실제 저장·인증 요청은 보내지 않습니다.';}));
  if(state==='disabled'||state==='loading')host.querySelectorAll('button,input').forEach(el=>el.disabled=true);
  if(state==='loading'){host.setAttribute('aria-busy','true');const notice=document.createElement('p');notice.setAttribute('role','status');notice.textContent='처리 중…';host.append(notice);}
 ...
 function render(){
  ...
  preview.replaceChildren();states.replaceChildren();
  ...
  else{preview.append(makePreview(selected,'default','main'));for(const state of Object.keys(names)){const card=document.createElement('article'),label=document.createElement('h3');label.textContent=names[state];card.append(label,makePreview(selected,state,'state-'+state));states.append(card);}$('#choose').disabled=false;}
...
export async function openDesignGallery(tools,{theme,framework='react',registry=[]},signal,{open=openWebPreview,timeout=300000}={}){
 ...
 const key=randomBytes(24).toString('hex'),nonce=randomBytes(18).toString('base64'),html=galleryHtml({theme,framework,registry},nonce);
 ...
 const server=http.createServer(async(req,res)=>{
  ...
  res.setHeader('Content-Security-Policy',`default-src 'none'; script-src 'nonce-${nonce}'; style-src 'unsafe-inline'; img-src data:; connect-src 'self'; form-action 'none'; frame-ancestors 'none'; base-uri 'none'`);
