...
export function galleryClient(data){
 ...
 let selected=data.initial||'button',search='',variant='solid',size='medium',framework=data.framework||'react',viewport=data.initial==='mobile-tabs'?'390':'fit',frame=null;const tokens={...data.theme};
 ...
 function makePreview(id,state,uid){
  ...
  for(const type of ['click','keydown','submit','input','focusin','focusout','mouseout'])host.addEventListener(type,event=>designAction(event,()=>{$('#activity').textContent='미리보기 동작을 확인했습니다. 실제 저장·인증 요청은 보내지 않습니다.';}));
 ...
 function mobilePreview(item){
  ...
  frame.srcdoc=mobilePreviewHtml(data.markup[selected],'.oc-design{'+vars(tokens)+'}'+data.styles,{variant,size,mobile:item.platform==='mobile-web',safeArea:$('#safe-area').checked,parentOrigin:location.origin},data.nonce);
