...
export function galleryClient(data){
 ...
 function makePreview(id,state,uid){
  ...
  for(const type of ['click','keydown','submit','input','change','focusin','focusout','mouseout','toggle','paste'])host.addEventListener(type,event=>{const emit=()=>{$('#activity').textContent='미리보기 동작을 확인했습니다. 실제 저장·인증 요청은 보내지 않습니다.';};designAction(event,emit);adminAction(event,emit);patternAction(event,emit);erpAction(event,emit);},type==='toggle');
...
.stage>.oc-design[data-admin=true],.stage>.oc-design[data-erp=true]{width:100%;max-width:960px}
