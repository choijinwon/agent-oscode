import {themeComponentCss,themeUsage} from './design-theme-component.js';
...
export function galleryClient(data){
 ...
 function render(){
  ...
  $('#reuse-styles').hidden=Boolean(item.team);
  renderReuse();
 ...
 function renderReuse(){
  $('#reuse-pages').replaceChildren();
  for(const [key,title,note]of [['shared','공통 스타일','한 번 만든 스타일을 여러 페이지에서 사용'],['page','페이지별 스타일','색상만 변경 · 나머지는 공통 스타일 유지']]){
   const card=document.createElement('article'),heading=document.createElement('h3'),caption=document.createElement('p'),wrapper=document.createElement('div');
   heading.textContent=title;caption.textContent=note;wrapper.dataset.ocTheme='gallery-shared';wrapper.dataset.reusePage=key;
   if(key==='page')wrapper.style.setProperty('--oc-theme-accent',pageAccent);
   const sample=makePreview('button','default','reuse-'+key);sample.removeAttribute('style');wrapper.append(sample);card.append(heading,caption,wrapper);$('#reuse-pages').append(card);
  }
  $('#theme-command').textContent='/design theme component '+framework+' src/components/DesignTheme'+data.extensions[framework];
  $('#theme-usage').textContent=data.usage[framework];
 }
 $('#page-accent').oninput=e=>{pageAccent=e.target.value;renderReuse();};
 for(const [button,target]of [['#copy-theme-command','#theme-command'],['#copy-theme-usage','#theme-usage']])$(button).onclick=async()=>{
  try{await navigator.clipboard.writeText($(target).textContent);$('#reuse-status').textContent='복사했습니다.';}
  catch{$('#reuse-status').textContent='복사 권한이 없습니다. 표시된 코드를 선택해 복사하세요.';}
 };
