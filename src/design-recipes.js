...
export function designRecipe(selection,filename){
 ...
 return {item,framework,title:designCatalog.find(x=>x.id===item).name,extension,code,css:designCss(tokens,scope,mobile,admin,interaction,erp),cssFile,minimum:designFrameworks[framework].minimum.join('.'),
  guidance:(erp?'ERP UI examples, not an SAP connector. Amounts use integer minor-unit strings, grouped by company and currency. Support KRW/JPY (0 decimals), USD/EUR (2); no FX, tax, negative postings or SAP configuration validation. erp-draft is a local validated payload, not a posted document; connect server validation and persistence explicitly. ':'')+(interaction?'Interactions are local examples. The action menu requires browser Popover API support. Step completion, archive and reordering emit events; connect persistence in the host app. ':'')+(admin?'Admin data is a local example. Implement invitation, routing, server pagination and server-side authorization in the host app; role choices do not enforce permissions. ':'')+'Native uncontrolled UI starter. Connect emitted action events to app data and backend. Form events expose the form; no credentials are logged or submitted by this starter. Angular requires a unique, stable id input. Gallery renders shared HTML behavior, not compiled framework components. Import component into the app and run its existing tests.'};
}
