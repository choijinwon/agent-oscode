...
export function designRecipe(selection,filename){
 ...
 if(erp)runtime+='\n'+[erpScale,erpParseAmount,erpFormatAmount,erpValidateJournal,erpUpdateGrid,erpAppendRow,erpUpdateLedger,erpAction].map(fn=>fn.toString()).join('\n')+'\nfunction erpDesignAction(event,emit=()=>{}) { designAction(event,emit); erpAction(event,emit); }';
 if(framework==='angular')runtime=runtime.replace(/function (designAction|adminAction|adminDesignAction|patternAction|patternDesignAction|erpAction|erpDesignAction)\(event,\s*emit=\(\)=>\{\}\)/g,'function $1(event: Event,emit: (detail: Record<string, unknown>) => void)').replace('function designMotion(root,node)','function designMotion(root: HTMLElement,node: Element | null)').replaceAll('filter(node=>node instanceof HTMLTableRowElement)','filter((node): node is HTMLTableRowElement=>node instanceof HTMLTableRowElement)').replaceAll('filter(node=>node instanceof HTMLInputElement)','filter((node): node is HTMLInputElement=>node instanceof HTMLInputElement)');
