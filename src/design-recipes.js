...
export function designRecipe(selection,filename){
 ...
 const html=markupFor(designMarkup[item],framework),attrs=`class="oc-design" data-design="${scope}" data-variant="${variant}" data-size="${size}" data-motion="${motion}"${mobile?' data-mobile="true"':''}${admin?' data-admin="true"':''}${interaction?' data-interaction="true"':''}`;
 let runtime=designAction.toString();
 const handlerName=admin?'adminDesignAction':interaction?'patternDesignAction':'designAction';
 ...
 if(framework==='angular')runtime=runtime.replace(/function (designAction|adminAction|adminDesignAction|patternAction|patternDesignAction)\(event,\s*emit=\(\)=>\{\}\)/g,'function $1(event: Event,emit: (detail: Record<string, unknown>) => void)').replace('function designMotion(root,node)','function designMotion(root: HTMLElement,node: Element | null)').replaceAll('filter(node=>node instanceof HTMLTableRowElement)','filter((node): node is HTMLTableRowElement=>node instanceof HTMLTableRowElement)').replaceAll('filter(node=>node instanceof HTMLInputElement)','filter((node): node is HTMLInputElement=>node instanceof HTMLInputElement)');
 const events="['click','keydown','submit','input','change','focusin','focusout','mouseout','toggle']";
 let code;
 ...
 return {item,framework,title:designCatalog.find(x=>x.id===item).name,extension,code,css:designCss(tokens,scope,mobile,admin,interaction),cssFile,minimum:designFrameworks[framework].minimum.join('.'),
  guidance:(interaction?'Interactions are local examples. The action menu requires browser Popover API support. Step completion, archive and reordering emit events; connect persistence in the host app. ':'')+(admin?'Admin data is a local example. Implement invitation, routing, server pagination and server-side authorization in the host app; role choices do not enforce permissions. ':'')+'Native uncontrolled UI starter. Connect emitted action events to app data and backend. Form events expose the form; no credentials are logged or submitted by this starter. Angular requires a unique, stable id input. Gallery renders shared HTML behavior, not compiled framework components. Import component into the app and run its existing tests.'};
}
