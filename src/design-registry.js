...
export function validateRegistry(data){
 ...
 const names=new Set();for(const item of data.items){
  ...
  for(const [key,limit]of [['code',64000],['css',18000],['description',400],['usage',1000]])if(typeof item[key]!=='string'||item[key].length>limit||item[key].includes('\0'))throw Error(`레지스트리 ${key} 오류`);
