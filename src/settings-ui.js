 ...
 try {
  ...
  while(true) {
   ...
   const action=await ui.choose('설정 · ↑↓ 선택 / Enter 열기',[
    ...(dependencies.approval?[{value:'approval',label:'승인 방식 (현재 에이전트에 즉시 적용)'}]:[]),
   ...
   try {
    if(action==='approval'){await dependencies.approval();notice='승인 방식은 현재 에이전트에 즉시 적용됩니다.';} else if(action==='style'){await dependencies.style();notice='스타일 설정은 모델 설정과 별도로 저장됩니다.';} else if(action==='chatgpt') {
