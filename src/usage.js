...
export function usageReport(session, detail = false) {
  ...
  const scoped = rows.at(-1)?.rules;
  if (scoped) lines.push(`마지막 요청 규칙: ${scoped.entries.filter(entry => entry.status === 'included').length}개 · 약 ${scoped.estimatedTokens} 토큰 (지침 항목에 포함) · 상세: /rules last`);
