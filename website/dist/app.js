...
search?.addEventListener('input',()=>{
 ...
 for(const link of links){const text=(link.textContent+' '+(link.dataset.docKeywords||'')).toLowerCase();link.hidden=!words.every(word=>text.includes(word));if(!link.hidden)count++;}
 for(const group of document.querySelectorAll('.sidebar .nav-group')){let item=group.nextElementSibling,visible=false;while(item&&!item.classList.contains('nav-group')){if(item.matches('[data-doc-link]')&&!item.hidden)visible=true;item=item.nextElementSibling;}group.hidden=!visible;}
